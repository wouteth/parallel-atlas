# Contributing to Parallel Atlas

Help make accounts of the past easier to compare and their sources easier to inspect. Contributions to research, writing, design, accessibility, and code are welcome. Read the [project goals](README.md#why-this-project-exists) and [Code of Conduct](CODE_OF_CONDUCT.md) first.
...
Search [existing issues](https://github.com/wouteth/parallel-atlas/issues) and [pull requests](https://github.com/wouteth/parallel-atlas/pulls) before starting. Small corrections can go straight to a pull request. For a new source track, major feature, data-model change, or broad refactor, open an issue first so the maintainer can help settle scope before substantial work.

Use the [issue chooser](https://github.com/wouteth/parallel-atlas/issues/new/choose) for bugs, feature proposals, and research corrections. Follow [SECURITY.md](SECURITY.md) for vulnerabilities; keep exploit details out of public issues.
...
The guest experience works without account credentials. Copy `.env.example` to `.env` only if you need local configuration. Never commit credentials, SQLite databases, browser storage, research downloads, or installed dependencies.
...
- Preserve permanent IDs because saved items and shared links depend on them.
...
Keep changes focused. Preserve responsive behavior, stable routes, guest/account data separation, and explicit date uncertainty. Prefer direct imports over re-export barrels. Write comments for non-obvious constraints and use commas, parentheses, or separate sentences instead of em dashes in new copy.
...
1. Commit a focused change with an imperative message, for example `fix(atlas): preserve uncertain date labels` or `docs(atlas): clarify source attribution`.
2. Push your branch to your fork and open a pull request against `wouteth/parallel-atlas:main`.
