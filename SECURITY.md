...
Parallel Atlas is an early research edition. Security fixes target the current `main` branch; there are no separately maintained release lines or guaranteed support windows. Operators of their own deployments are responsible for configuration, dependency updates, and user data.

The account API includes OIDC integration, sessions, and SQLite storage. Live identity-provider verification and production deployment work remain documented in [HANDOFF.md](HANDOFF.md).

## Report a vulnerability privately

Use [GitHub's private vulnerability reporting form](https://github.com/wouteth/parallel-atlas/security/advisories/new). Private reporting is enabled for this repository. Do not open a public issue or pull request containing exploit details, credentials, session cookies, or other people's data.
